    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 flex items-center gap-2.5 border-b border-sidebar-border">
        ...
        <div className="min-w-0">
          <div className="font-semibold text-sidebar-foreground text-sm truncate">{t("app.name")}</div>
      ...
      <div className="p-3 border-t border-sidebar-border space-y-2">
        <div className="px-3 py-2 rounded-lg bg-sidebar-accent/40">
          <div className="text-xs text-sidebar-foreground/60">{t("common.signedInAs")}</div>
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            ...
            {roles.map((r) => (
              <span
                key={r}
                className="text-[10px] px-1.5 py-0.5 rounded gradient-brand text-primary-foreground"
              >
                {t(`role.${r}`)}
              </span>
            ))}
        ...
        <Button variant="ghost" onClick={handleSignOut} className="w-full justify-start text-sidebar-foreground/80 hover:text-sidebar-foreground">
          <LogOut className="w-4 h-4 mr-2 rtl:mr-0 rtl:ml-2" /> {t("common.signOut")}
        </Button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex bg-background" dir={dir}>
      ...
      {openMobile && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className={cn(
            "absolute top-0 bottom-0 w-72 bg-sidebar border-sidebar-border",
      ...
      <main className="flex-1 min-w-0">
        <header className="flex items-center justify-between px-4 h-14 border-b border-border bg-card/30">
          <button onClick={() => setOpenMobile(true)} className="lg:hidden p-2 rounded-md hover:bg-muted">
